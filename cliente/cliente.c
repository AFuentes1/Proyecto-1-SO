...
void* request_file(void* arg) {
    ...
    // Detectar si el usuario ingresó una request HTTP ya formateada
    if (strncmp(filename, "GET ", 4) == 0) {
        snprintf(request, sizeof(request), "%s", filename);
        printf("request HTTP enviada directamente: %s\n", filename);
    } else {
        snprintf(request, sizeof(request), "%s", filename);
        printf("request plana/directa enviada: %s\n", filename);
    }
